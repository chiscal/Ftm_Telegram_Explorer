...
export class Transaction {
    private static provider(network: string) {
        return network.toLowerCase() == "mainnet"
            ? new ethers.JsonRpcProvider(config.mainnet.rpc, {
                  name: config.mainnet.name,
                  chainId: config.mainnet.chainId,
              })
            : new ethers.JsonRpcProvider(config.testnet.rpc, {
                  name: config.testnet.name,
                  chainId: config.testnet.chainId,
              });
    }

    public static toDateTime(secs: number) {
    ...
    static async getTxnInfo(hash: string, network: string) {
        try {
            ...
            let status =
                (await txn?.wait())?.status == 1
                    ? "SUCCESS"
                    : (await txn?.wait())?.status == 0
                    ? "FAIL"
                    : "PENDING";
            let fee = ethers.formatEther(
                String(
                    Number(txn?.gasPrice?.toString()) *
                        Number((await txn?.wait())?.gasUsed.toString())
                )
            );
            ...
            async function getLogs(log: Log) {
                let checker = (
                    await Token.getTokenInfo(String(log.address), network)
                )?.decimals;
                if (typeof checker == "number" && log.topics[2] != undefined) {
                    ...
                    let token_amt = Number(
                        Number(hex_val / 10 ** Number(token_decimal)).toFixed(5)
                    );
                    var tokenTransferInfo = {
                        name: token_sym,
                        from: String(txn?.from),
                        to: token_txns_addr,
                        amount: token_amt.toFixed(6),
                    };
            ...
            return {
                status: status,
                sender: String(txn?.from),
                reciever: to_addr,
                amount: coin_amt,
                time: time,
                fee: Number(Number(fee).toFixed(5)),
